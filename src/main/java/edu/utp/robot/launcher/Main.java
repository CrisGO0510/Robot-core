...
import edu.utp.robot.domain.module.staticmod.actuation.Actuator;
import edu.utp.robot.domain.module.staticmod.perception.Camera;
import edu.utp.robot.domain.module.staticmod.perception.SensorProximity;
...
public class Main {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(
                () -> {
                    ...
                    Robot robot =
                            new Robot(
                                    config.getRobotSerie(),
                                    config.getRobotAlias(),
                                    config.getRobotDescripcion());
                    RobotServices robotServices =
                            new RobotServices(robot, extension, rotation, helical);
                                    ...
                                    for (int i = 0; i < 10; i++) {
                                        for (int j = 0; j < 10; j++) {
                                            ...
                                            if (worldMap.hayObstaculo(i, j)) {
                                                ...
                                                g.fillRect(
                                                        x + 5, y + 5, cellSize - 10, cellSize - 10);
                                            }
                    ...
                    btnAvanzarInteligente.addActionListener(
                            e -> {
                                if (!robotEncendido[0]) {
                                ...
                                try {
                                    ...
                                    JOptionPane.showMessageDialog(
                                            frame, "Número de pasos inválido.");
                                    return;
                                }
                                moverInteligente(
                                        pasos,
                    ...
                    btnSensor.addActionListener(
                            e -> {
                                sensorProximity.encender();
                                Object lectura = sensorProximity.captarInformacion();
                                lblSensor.setText(
                                        "Sensor: " + (lectura != null ? lectura : "Sin lectura"));
                                sensorProximity.apagar();
                            });

                    btnCamara.addActionListener(
                            e -> {
                                camera.encender();
                                Object captura = camera.captarInformacion();
                                Object resultado = camera.procesarDatos(captura);
                                lblCamara.setText(
                                        "Cámara: " + (resultado != null ? resultado : "Sin datos"));
                                camera.apagar();
                            });

                    btnActuador.addActionListener(
                            e -> {
                                actuator.encender();
                                int res = actuator.realizarAccion();
                                lblActuador.setText(
                                        "Actuador: "
                                                + (res == 1 ? "Acción realizada" : "No se pudo"));
                                actuator.apagar();
                            });

                    btnComunicar.addActionListener(
                            e -> {
    ...
    private static void moverInteligente(
        ...
        new Thread(
                        () -> {
                            Position anterior =
                                    new Position(worldMap.getRobotX(), worldMap.getRobotY());
                            int pasosDados = 0;
                            int intentos = 0;
                            int maxIntentos = 30;
                            ...
                            while (pasosDados < pasos && intentos < maxIntentos) {
                                ...
                                if (animales.contains(actual)) {
                                    ...
                                    JOptionPane.showMessageDialog(
                                            frame, "¡Animal detectado! Sonido ahuyentador.");
                            ...
                            if (intentos >= maxIntentos) {
                                SwingUtilities.invokeLater(
                                        () ->
                                                JOptionPane.showMessageDialog(
                                                        frame,
                                                        "El robot no puede avanzar más y ha dejado"
                                                                + " de intentar."));
                            }
                        })
                .start();
